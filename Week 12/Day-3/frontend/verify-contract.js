...
const {
  CLAW_TOKEN_ADDRESS,
  CALW_TOKEN_ABI,
} = require("./src/config/contract.ts");
...
async function verifyContract(chain) {
  ...
  console.log(`📍 Contract Address: ${CLAW_TOKEN_ADDRESS}`);
  ...
  try {
    ...
    const code = await client.getBytecode({ address: CLAW_TOKEN_ADDRESS });
...
async function main() {
  ...
  if (!foundOnKasplex && !foundOnSepolia) {
    ...
    console.log("2. Update CLAW_TOKEN_ADDRESS in src/config/contract.ts");
    console.log(
