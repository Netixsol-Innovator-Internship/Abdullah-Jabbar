...
export default function FollowersModal({ title, initialUsers, onClose }: FollowersModalProps) {
  ...
  useEffect(() => {
    if (!socket) return;
    const onFollowLocalEcho = (payload: { targetUserId: string; isFollowing: boolean }) => {
      setUsers((prev) =>
        prev.map((u) => (u._id === payload.targetUserId ? { ...u, isFollowing: payload.isFollowing } : u)),
      );
    };
    socket.on('follow.changed', onFollowLocalEcho);
    return () => socket.off('follow.changed', onFollowLocalEcho);
  }, [socket]);
  ...
  const toggleFollow = useCallback(
    async (targetId: string, current: boolean) => {
      ...
      // optimistic
      setUsers((prev) => prev.map((u) => (u._id === targetId ? { ...u, isFollowing: !current } : u)));

      try {
        const res = await API.post(`/followers/toggle/${targetId}`);
        const serverIsFollowing =
          typeof res?.data?.isFollowing === 'boolean' ? res.data.isFollowing : !current;
        // align with server
        setUsers((prev) => prev.map((u) => (u._id === targetId ? { ...u, isFollowing: serverIsFollowing } : u)));
        // notify other components for global consistency
        socket?.emit('follow.changed', { targetUserId: targetId, isFollowing: serverIsFollowing });
      } catch {
        // rollback
        setUsers((prev) => prev.map((u) => (u._id === targetId ? { ...u, isFollowing: current } : u)));
      } finally {
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50">
      <div className="bg-white rounded-lg w-full max-w-md shadow-md">
        ...
        <div className="max-h-[60vh] overflow-y-auto p-3">
          {users.length === 0 && (
            <div className="text-center text-gray-500 text-sm py-8">No users to show.</div>
          )}
          <ul className="space-y-3">
            {users.map((u) => (
              <li key={u._id} className="flex items-center">
                <img
                  src={u.profilePicture ? `/uploads/${u.profilePicture}` : '/images/avatar.png'}
