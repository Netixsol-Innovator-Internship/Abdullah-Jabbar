...
export default function SummaryCard({ document }: { document: Document }) {
  return (
    <div className="p-4 rounded-2xl shadow bg-white" role="region" aria-labelledby="summary-heading">
      <div className="flex items-center mb-3">
        <FileText className="h-5 w-5 text-gray-500 mr-2" />
        <h2 id="summary-heading" className="text-xl font-semibold text-gray-800">Document Information</h2>
      </div>

      <div className="space-y-2 text-sm mb-4">
        <div>
          <span className="text-gray-500">File:</span>
          <span className="ml-2 text-gray-700">{escapeHtml(document.fileName)}</span>
        </div>
        <div>
