...
export default function ChatBox({
    ...
    try {
      ...
      const response = await submitQuery({
        ...
        onChunk: (chunk) => {
          ...
          streamedText += chunk;
          setMessages((prev) =>
            prev.map((m) => (m.id === systemId ? { ...m, content: streamedText } : m))
          );
        },
      });
      ...
      setMessages((prev) => prev.map((m) => (m.id === systemId ? finalMessage : m)));
      if (onUpdateDocument) onUpdateDocument(finalMessage);
    } catch (err) {
      setMessages((prev) =>
        prev.map((m) =>
          m.id.startsWith('sys-') ? { ...m, content: 'Sorry, I encountered an error processing your question. Please try again.' } : m
        )
      );
    } finally {
      setIsQuerying(false);
  ...
  return (
    <div className="flex flex-col h-full">
      <div className="flex-1 overflow-auto mb-4 space-y-4" role="log" aria-live="polite">
        {messages.map((message) => (
          <div key={message.id} className={`flex ${message.type === 'user' ? 'justify-end' : 'justify-start'}`}>
            <div className={message.type === 'user' ? 'self-end bg-blue-100 rounded-xl p-3 max-w-[75%]' : 'self-start bg-gray-100 rounded-xl p-3 max-w-[75%]'}>
      ...
      <form onSubmit={handleSubmit} className="mt-auto">
        <label htmlFor="chat-input" className="sr-only">Ask a question about the document</label>
